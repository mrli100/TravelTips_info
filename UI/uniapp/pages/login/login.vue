	...
	<view>
		...
		<view class="login">
			<view class="content">
				...
				<!-- 主体表单 -->
				<view class="main">
					<wInput v-model="phoneData" type="text" maxlength="11" placeholder="用户名/电话" :focus="isFocus">
					</wInput>
	...
	export default {
		data() {
			return {
				...
				phoneData: 'mrlib100', //用户/电话
				passData: 'admin@123', //密码
		...
		mounted() {
			_this = this;
			// #ifdef MP-WEIXIN
			let is_new_login = uni.getStorageSync('is_new_login');
			if (is_new_login != null && is_new_login != '' && typeof(is_new_login) != "undefined") {
		...
		methods: {
			loset(Logon) {
				let is_new_login = uni.getStorageSync('is_new_login');
				util.request(api.loginUsersInfo, {
			...
			startLogin(e) {
				...
				util.request(api.loginEmpowerUrl, {
