	...
	<view>
		<view class="card-item" @click="toDynamic">
			...
			<view class="card-item-info">
				...
				<view class="item-info-file" v-if="item.type==1">
				...
				<view class="item-info-file" v-if="item.type==2">
					<view class="info-h-w" :class="{'info-w-h':(item.video.wide*1) > (item.video.high*1)}">
						<lazy-load :src="item.video.img" border-radius="8rpx"></lazy-load>
						<view class="info-video df">
							<image src="/static/img/v.png"></image>
						</view>
					</view>
				</view>
				<view class="item-info-content ohto2">{{item.content}}</view>
				<view class="df" v-if="item.circle_name || item.adds">
					<view v-if="item.circle_name" class="info-topic df" @click.stop="toCircle">
		...
		methods: {
			likeClick() {
				...
				util.request(api.likeDynamicUrl, {
					...
					img: that.item.img_count>0 ? that.item.img[0].url : '',
				}, 'POST');
			},
			toDynamic() {
				uni.navigateTo({
...
<style lang="scss" scoped>
	...
	.card-item-info .info-h-w .info-video,
	.card-item-info .info-w-h .info-video {
		position: absolute;
		right: 20rpx;
		top: 20rpx;
		justify-content: center;
	...
	.card-item-info .info-h-w .info-video image,
	.card-item-info .info-w-h .info-video image {
		width: 32rpx;
		height: 32rpx;
	}
